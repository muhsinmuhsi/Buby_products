...
const Dashboard = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const allorder= await axios.get('http://localhost:3000/selledProducts')
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-3 gap-4 text-white">
        <div className="bg-indigo-500 p-4 shadow rounded-lg">
          ...
          <p className="text-sm">Recent: {users[users.length - 1]?.name || "N/A"}</p>
        </div>
        <div className="bg-indigo-500 p-4 shadow rounded-lg">
          <h2 className="text-xl font-bold mb-2">Total Products</h2>
          <p className="text-2xl">{products.length}</p>
          <p className="text-sm">Recent: {products[products.length - 1]?.name || "N/A"}</p>
        </div>
        <div className="bg-indigo-500 p-4 shadow rounded-lg">
          <h2 className="text-xl font-bold mb-2">Total Orders</h2>
          <p className="text-2xl">{allorders.length}</p>
          <p className="text-sm">last order: {products[allorders.length - 1]?.name || "N/A"}</p>
        </div>
      </div>
      {/* Quick Links Section */}
      <div className="mt-6">
